...
export function initTerminalWindow(elem, { fontSize }) {
  ...
  term.open(elem);
  term.writeln(
    `${termColor.TERM_TEXT_GREEN}[TERM] ${termColor.TERM_TEXT_GRAY}欢迎来到实例控制台${termColor.TERM_TEXT_DARK_AQUA}`
  );
  term.writeln(
    `${termColor.TERM_TEXT_GREEN}[TERM] ${termColor.TERM_TEXT_GOLD}控制台历史记录加载中...${termColor.TERM_RESET}\r\n`
  );
  term.info = (msg) => {
